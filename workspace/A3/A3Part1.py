...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    """
    ## Your code here
    # compute the periods of the two sinusoids
    import math 

    T1 = fs / f1
    T2 = fs / f2 
    # print(f"T1 {T1}, T2 {T2}, gcd: {math.gcd(int(T1), int(T2))}")
    # get the LCM of T1, T2
    M = int ((T1 * T2) /  math.gcd(int(T1), int(T2)) )
    
    # keep the first M samples of x
    xM = x[:M]
    # get the DFT
    X = fft(xM)
    # get the magnitude of the first half of X; use decibels
    mX = 20 * np.log10( np.abs(X[:M//2 + 1]) )
    # zero-out the decibels less than -120 dB
    mX[mX < -120 ] = -200.0
