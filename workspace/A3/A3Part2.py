...
def optimalZeropad(x, fs, f):
    ...
    # get the period
    period = fs / f
    # find the min N that is >= M, and is an int multiple of the period
    N = np.ceil(M / period) * period
    N = int(N)

    hM1 = int(math.floor((M+1)/2))
    hM2 = int(math.floor(M/2))

    print(f"len M={M}, N={N}, period={period}, fs={fs}, f={f}, hM1={hM1}, hM2={hM2}")


    # zero pad x, with centering
    x_padded = np.zeros(N)
    x_padded[:hM1] = x[hM2:]
    x_padded[N-hM2:] = x[:hM2]

    # get FFT of the zero-padded signal
    X = fft(x_padded)

    # get positive half of the mag spectrum; in decibels
    mX = 20 * np.log10( abs(X[: (N//2) + 1]))

    # set values < -120dB to less than -120dB for precision
    mX[mX < -120] = -120.
...
## Test case 
# x = np.arange(1,459)
# X = optimalZeropad(x, 10000.0, 200.0)
# print(f"x len={len(x)}")
# print(f"X len={len(X)}")
# print(f"max mx {max(X)}, min mx {min(X)}")