...
def suppressFreqDFTmodel(x, fs, N):
    ...
    print(f"M {M}, fs {fs}, w_len {w.size}, N {N}")
    
    ## Your code here
    # get the analysis of the signal
    mX, pX = dftAnal(x, w, N)

    # convert 70Hz to bin index
    bin70Idx = int(np.ceil(70.0 * N / fs))

    # copy mag spectra
    mX_filtered = mX.copy()
    # suppress all freq above 70hz; set the mag to a low value below -120 dB
    mX_filtered[:bin70Idx + 1] = -120   # some low low value

    # synthesize the signal back
    y = dftSynth(mX, pX, w.size) * outputScaleFactor 
    yfilt = dftSynth(mX_filtered, pX, w.size) * outputScaleFactor 
    ...
    # plt.subplot(4,1,2)
    # plt.plot(mX_filtered); plt.title('mx_filter')

    # plt.subplot(4,1,3)
    # plt.plot(y); plt.title('y')

    # plt.subplot(4,1,4)
    # plt.plot(yfilt); plt.title('y_filtered')
    # plt.tight_layout()
    # plt.show()
...
N = 1024   # FFT size
t = np.arange(0, 1, 1.0/fs)  # 1-second duration time vector

# Create a signal with components 40 Hz, 100 Hz, 200 Hz, 1000 Hz
x = 0.5 * np.sin(2 * np.pi * 40 * t) + 0.5 * np.sin(2 * np.pi * 100 * t) \
    + 0.5 * np.sin(2 * np.pi * 200 * t) + 0.5 * np.sin(2 * np.pi * 1000 * t)

# Apply the DFT model suppression
y, yfilt = suppressFreqDFTmodel(x, fs, N)

# Print results
print("Test Case 1: Signal with 40 Hz, 100 Hz, 200 Hz, 1000 Hz components")
print("Original DFT model output (first 10 samples):", y[:10])
print("Filtered DFT model output (first 10 samples):", yfilt[:10])
