...
def testRealEven(x):
    ...
    # Check the symmetry properties of the DFT for real and even signals
    # For a real and even signal, the imaginary part of X should be zero (or very small due to precision issues)
    # Check if the imaginary part of the DFT is within the error tolerance
    isRealEven = np.all(np.abs(np.imag(X)) < 1e-6)
    ## NOTE - very important!! convert to Python bool from np.bool_
    # Otherwise the submission fails due to JSON encoding
    isRealEven = bool(isRealEven) # convert to Python bool
...
print(testRealEven(x1))  # Expected: (True, zero-phase dftbuffer, DFT of dftbuffer)
print(testRealEven(x2))  # Expected: (False, zero-phase dftbuffer, DFT of dftbuffer)
"""